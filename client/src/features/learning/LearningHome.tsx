...
export function LearningHome() {
	...
	const sessions = useMemo(() => {
		const mergedSessions = pageResponses.flatMap(
	...
	const handleRevise = useCallback(
		async (sessionId: string, mode: "full_review" | "quiz_only") => {
			try {
				const revisionSession = await createRevisionSession(sessionId, {
					mode,
				});
	...
	const isInitialLoad =
		(isLoading && offset === 0 && sessions.length === 0) ||
		isAllCoursesLoading ||
		isAllCoursesError;
	const showDashboard =
		hasAnyCourses || isAllCoursesLoading || isAllCoursesError;
			...
			>
				...
				<div className={cn("text-center", showDashboard ? "mb-6" : "mb-8")}>
					...
					{!showDashboard && (
						<p className="text-lg text-muted-foreground max-w-md mx-auto">
							Enter a topic and master it through guided explanations and
				...
				{showDashboard && (
					...
					>
						...
						>
							<h2 id="your-courses-heading" className="text-xl font-semibold">
								Your Courses
							</h2>
							<ChevronDown
								className={cn(
													...
													{sessions.map((session, index) => (
														<motion.div
															...
															transition={{
																...
																damping: 25,
																delay:
																	index < COURSES_PER_PAGE ? index * 0.05 : 0,
															}}
														>
															<CourseCard
																session={session}
