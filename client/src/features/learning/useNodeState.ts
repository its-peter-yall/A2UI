 ...
 * LOCATION: client/src/features/learning/useNodeState.ts
 * ============================================================================
 *
 * PURPOSE:
 ...
 *    flow and exports utility functions for state transition validation.
 *
 * ROLE IN PROJECT:
 *    Used by UI components to enable/disable buttons and determine which view
 *    to render. Acts as the client-side mirror of server-side transition rules,
 *    providing optimistic UX guidance without being authoritative.
 *
 * KEY COMPONENTS:
 *    - useNodeState: Returns status, actions, computed flags, currentView
 *    - isValidTransition: Validates status-to-status transitions
 ...
 * USAGE:
 *    ```tsx
 *    const { actions, currentView } = useNodeState(node, quizResult);
 *    {actions.canProceedToQuiz && <Button onClick={proceedToQuiz}>Proceed</Button>}
 *    ```
 * ============================================================================
 */
